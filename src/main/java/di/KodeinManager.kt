...
class KodeinManager {
    ...
    val kodein = Kodein {
        ...
        bind<ChampionDAO>() with provider { ChampionDAO(kodein) }
        bind<BanDAO>() with provider { BanDAO(kodein.instance()) }
        bind<MasteryDAO>() with provider { MasteryDAO(kodein.instance()) }
        bind<PlayerDAO>() with provider { PlayerDAO(kodein.instance()) }
        bind<RuneDAO>() with provider { RuneDAO(kodein.instance()) }
        bind<StatDAO>() with provider { StatDAO(kodein.instance()) }
        bind<TeamDAO>() with provider {
            TeamDAO(
                kodein.instance(),
                kodein.instance())
        }
        bind<GameStageDeltaDAOImpl>() with provider { GameStageDeltaDAOImpl(kodein.instance()) }
